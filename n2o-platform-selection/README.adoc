...
В API описаны два интерфейса `Selection<E>` и `Fetcher<E>`.
...
Имея данные реализации их можно использовать через метод `net.n2oapp.platform.selection.core.Selector.resolve(Fetcher<E>, Selector<E>)`.

Данный метод в начале проверит, что переменную типа `E` интерфейса `Selection<E>` можно присвоить переменной типа `E` интерфейса `Fetcher<E>`.
...
== Генерация интерфейсов `Fetcher` и `Selection` по `DTO`
...
Генератор кода распознает данную аннотацию и сгенерирует интерфейсы `Selection` и `Fetcher` для каждого `DTO`.
