...
Последняя указывает, что в дефолтных реализациях `Selection` при наличии наследования между ними в подклассах следует переопределять методы. Например, если есть:
[source,java]
----
...
Это позволяет использовать `call-chain` без кастов. Например:
[source,java]
----
...
Так же стоит иметь в виду, что спец-символы JSON при передаче по сети будут кодироваться процентами.
