...
У генератора кода есть две опции `net.n2oapp.platform.selection.addJacksonTyping` и `net.n2oapp.platform.selection.addJaxRsAnnotations`.
Первая добавляет типизацию для Jackson-а, вторая -- аннотации JAX-RS (позволяет передавать `Selection`-ы в параметрах запроса HTTP).
...
Невозможно через параметры запроса указать (по крайней мере в JAX-RS),
...
Если ничего из этого не удается, единственный выход -- нарушить соглашения по построению REST-сервисов и отдавать
данные по POST запросу с Body. Так вы можете описать произвольно сложную выборку в формате JSON.
